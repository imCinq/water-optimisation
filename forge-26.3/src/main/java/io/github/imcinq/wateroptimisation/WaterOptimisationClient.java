...
@Mod(WaterOptimisationClient.MOD_ID)
public final class WaterOptimisationClient {
	...
	public WaterOptimisationClient(FMLJavaModLoadingContext context) {
		...
		context.registerExtensionPoint(
				...
				() -> new ConfigScreenHandler.ConfigScreenFactory(parent -> new WaterOptimisationScreen(parent))
		);
	}

	private void onClientSetup(FMLClientSetupEvent event) {
		sodiumLoaded = ModList.isLoaded("sodium");
		ConfigManager.load();
	...
	/** Forge 26.3's vanilla fluid renderer has the reviewed optional reverse-face hook. */
	...
	/** Describes the renderer path available to the screen's current unsaved settings. */
	...
	public static boolean shouldKeepWaterParticle(ParticleOptions particle, boolean alwaysShow, double x, double y, double z) {
		...
		if (!settings.active() || !isWaterParticle(particle)) {
			return true;
		}
		...
		if (reference.available() && !WaterParticleDistancePolicy.isWithinDistanceSquared(
				settings.maxDistanceSquared(), reference.x(), reference.y(), reference.z(), x, y, z
		)) {
			if (diagnosticsEnabled) {
	...
	/** Drawn immediately after Forge's vanilla HUD extraction by ForgeGuiMixin. */
	public static void renderDiagnostics(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
