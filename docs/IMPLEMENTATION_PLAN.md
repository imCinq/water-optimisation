...
- Fabric 26.3;
- Fabric 1.21.11;
- NeoForge 26.3 as a separate engineering preview, promoted after its runtime
  and build tooling stabilise.
...
5. maintain the target-isolated NeoForge 26.3 port and promote it after a usable runtime coordinate is available;
...
The first four settings/UI steps are now implemented in source: the 1.21.11 profile compiles, 26.3 is the default target, the shared presentation rules drive all current adapters, and the Advanced screen has bounded scrolling with a fixed footer. The NeoForge 26.3 port baseline is now present under `neoforge-26.3/`; its release is blocked only by the usable preview runtime coordinate and the remaining target validation.
