...
| --- | --- |
| Mod version | 0.0.6 |
| Minecraft | 26.2 and 1.21.1, with target-specific 0.0.6 artifacts |
...
Use the published [GitHub Release](https://github.com/imCinq/water-optimisation/releases/tag/v0.0.6) for the target-specific runtime JARs, matching sources JARs, and `SHA256SUMS-0.0.6.txt`. Choose the asset matching the Minecraft target, then verify the tag, accepted commit, and checksum before distributing the artifact. The former `v0.0.5` release remains available as historical distribution.
