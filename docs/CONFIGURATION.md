...
| flatWaterSurfaceMeshing | false | 26.2-only prototype currently disabled for visual safety. A single atlas quad cannot preserve vanilla's repeated water texture without shader-side tiling, so this option is unavailable until that can be implemented. It is also unavailable on 1.21.1. |
