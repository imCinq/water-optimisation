...
| farWaterPass | false | 26.2-only experiment. In conservative water-only sections, draws owned still-water geometry through a separate terrain pass and omits eligible sections beyond 64 blocks. It is a hard cutoff, disabled by default, unavailable with Sodium, and unavailable on 1.21.1. Mixed translucent, flowing, waterlogged, transparent, overlay, and ambiguous cases stay on the normal path. |
