...
- Water rendering: the fluid-geometry mode, the optional reduced-face setting, the deferred flat-surface prototype, and the guarded far-water pass;
...
The labels are phrased as short questions so their effect is understandable without renderer knowledge: “Skip hidden water blocks?”, “Limit water particles per tick?”, and “Combine flat water surfaces?”. The flat-surface control is currently disabled because a single atlas quad cannot preserve vanilla's repeated water texture. Sodium ownership displays a short notice and disables the overlapping vanilla geometry controls unless the reviewed Sodium 0.9.x/Minecraft 26.2 face bridge has matched; on 1.21.1, the flat-surface experiment remains unavailable while the particle and diagnostic controls stay available. The effective-path summary is based on the unsaved working copy, so changing a preset immediately explains what Apply will do.

The far-water toggle is labelled `Limit distant water to 320 blocks?` so its effect is explicit. It is disabled by default and unavailable with Sodium or on 1.21.1. It only activates after a section-level preflight proves that the section contains ordinary still water without mixed non-solid model geometry; the pass owns upward surfaces and leaves sides/bottoms on the normal renderer. It uses the current camera state and translucent target each frame, and sorts owned sections back-to-front, so the owned mesh follows movement without taking over unrelated translucent geometry. This is a hard distance cutoff, not a fade, and it must be visually and quantitatively reviewed before being promoted into a preset.
