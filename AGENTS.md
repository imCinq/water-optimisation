...
Client-only behavior is not automatically approved by every server. Check the current server rules before use.
...
7. State which checks ran in CI and which ran in a local Minecraft client.

## Change scope

Keep commits focused. Update the relevant documentation and changelog entry with implementation changes. Do not publish a release until the target artifact has passed the documented visual and compatibility checks.
