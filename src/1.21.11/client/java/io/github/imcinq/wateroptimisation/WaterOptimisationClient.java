...
public final class WaterOptimisationClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		HudElementRegistry.addLast(
				...
				WaterOptimisationClient::renderDiagnostics
		);

		if (sodiumLoaded) {
			LOGGER.info("Sodium detected on Minecraft 1.21.11; vanilla fluid optimization hooks are disabled and this target remains on the particle-only fallback.");
		}
		LOGGER.info("Water Optimisation initialized for Minecraft 1.21.11; rendering changes are opt-in and default to safe no-op behavior.");
	}
	...
	/** The 1.21.11 renderer has no reviewed reverse-face hook yet. */
	...
	private static void renderDiagnostics(GuiGraphics graphics, DeltaTracker deltaTracker) {
		...
		if (diagnosticsLines.length == 0 || now >= diagnosticsRefreshDeadlineNanos) {
			...
			diagnosticsLines = new Component[]{
				...
				Component.literal("water backfaces: " + (sodiumLoaded ? "Sodium-owned" : "vanilla (1.21.11 compatibility)")),
